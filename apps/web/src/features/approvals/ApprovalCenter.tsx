...
export function ApprovalCenter({ approvals, hunks, onResolveApproval, onResolveHunk, onApplyPatch, patchApplyState }: ApprovalCenterProps) {
  return (
    <section className="tool-panel" aria-label="Approvals">
      ...
      <div className="space-y-3">
        {approvals.length === 0 ? <div className="empty-state compact">No pending approvals.</div> : null}
        {approvals.map((approval) => (
          <article key={approval.id} className="approval-card">
            <div className="flex items-start gap-3">
              <div className="approval-icon">{approval.kind === "applyPatch" ? <KeyRound size={17} /> : <Clock size={17} />}</div>
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  <h3 className="truncate font-medium text-ink-900">{approval.summary}</h3>
      ...
      <PatchReview hunks={hunks} onResolveHunk={onResolveHunk} onApplyPatch={onApplyPatch} applyState={patchApplyState} />
    </section>
  );
}
