...
async function writeAppServerScript(
  ...
  await writeFile(
    ...
    [
      ...
      "  if (message?.method === 'turn/completed' && message.params?.turn?.status === 'completed') {",
      "    process.stdout.write(`${JSON.stringify({ method: 'thread/status/changed', params: { threadId: message.params.threadId ?? 'thread-1', status: { type: 'idle' } } })}\\n`);",
      "  }",
      "}",
      "process.stdin.on('data', (chunk) => {",
      "  buffer += chunk;",
